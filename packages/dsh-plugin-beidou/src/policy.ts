 ...
 * 第二道为身份门禁:未登录拒绝业务工具;身份工具(login/status)始终放行。
...
/** 北斗注册的 8 个业务工具(正向白名单;新增工具必须显式登记于此) */
export const BUSINESS_TOOLS = new Set([
  "search_semantics", "query_metrics", "query_dataset", "clarify",
  "diagnose_metric", "search_knowledge", "read_playbook", "list_ontology",
]);
...
export function decideToolAccess(input: PolicyInput): PolicyDecision {
  ...
  if (!ALLOWED_TOOLS.has(input.tool)) {
    return {
      ...
      reason: `工具 ${input.tool} 不在北斗白名单内(白名单=8 业务+2 身份;此拒绝通常意味着 profile 配置漂移或越权注册)`,
    };
  }
