...
import { ERROR_CODES } from "@beidou/contracts/src/index.ts";
...
describe("decideToolAccess(策略裁决)", () => {
  ...
  it("白名单恰为 9 业务 + 2 身份", () => {
    expect(BUSINESS_TOOLS.size).toBe(9);
    expect(ALLOWED_TOOLS.size).toBe(11);
  });
  it("身份工具始终放行(否则无法登录)", () => {
  ...
  it("已配置企业服务凭据 → 只读指标链路匿名可用,数据权限工具仍需登录", () => {
    expect(decideToolAccess({ tool: "query_metrics", serviceAuth: true }).decision).toBe("allow");
    expect(decideToolAccess({ tool: "search_semantics", serviceAuth: true }).decision).toBe("allow");
    expect(decideToolAccess({ tool: "query_dataset", serviceAuth: true }).decision).toBe("deny");
  });
});
...
describe("toBeidouResult(单轨转换)", () => {
  ...
  it("结构化结果透传口径与血缘证据", () => {
    const r = toBeidouResult(
      {
        ok: true,
        text: JSON.stringify({ metric: "metric_102487", columns: ["metric_value"], rows: [{ metric_value: 12 }] }),
        evidence: [{ kind: "metric_query", caliber: "sum(amount)", sql: "SELECT sum(amount)", physicalTables: ["internal.dwd.fact_store_orders"], semanticVersion: "service-domain@r1" }],
      },
      { tool: "query_metrics", traceId: "btr-evidence" },
    );
    const result = (r as { data?: { analysisResults?: Array<{ evidence?: Array<Record<string, unknown>> }> } }).data?.analysisResults?.[0];
    expect(result?.evidence?.[0]).toMatchObject({ traceId: "btr-evidence", caliber: "sum(amount)", sql: "SELECT sum(amount)", physicalTables: ["internal.dwd.fact_store_orders"], semanticVersion: "service-domain@r1" });
  });
  it("诊断归因生成可视化柱状结果", () => {
    const r = toBeidouResult(
      {
        ok: true,
        text: JSON.stringify({ metric: "metric_102487", anomaly: { anomaly: true, deltaPct: -12 }, totals: { current: 88, prior: 100, delta: -12 }, attributions: [{ dim: "dimension.city", top: [{ value: "上海", contribution: -0.12 }] }] }),
        evidence: [{ kind: "metric_query", semanticVersion: "service-domain@r1" }],
      },
      { tool: "diagnose_metric", traceId: "btr-chart" },
    );
    const results = (r as { data?: { analysisResults?: Array<Record<string, unknown>> } }).data?.analysisResults ?? [];
    expect(results.map((item) => item.type)).toEqual(["diagnosis", "bar"]);
    expect(results[1]).toMatchObject({ title: "维度贡献 · dimension.city", labels: ["上海"], series: [{ values: [-12] }] });
  });
