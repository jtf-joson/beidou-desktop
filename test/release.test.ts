...
describe('GitHub release contract', () => {
  ...
  it('packages an isolated development channel from the current workspace', async () => {
    ...
    expect(developmentConfig.productName).toBe('北斗work')
    ...
    expect(developmentConfig.artifactName).toBe('beidou-work-dev-${os}-${arch}.${ext}')
    expect(developmentConfig.nsis.artifactName).toBe('beidou-work-dev-windows-${arch}-setup.${ext}')
  })
