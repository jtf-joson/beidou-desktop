...
export interface HarnessRuntimeOptions {
  ...
  /** 北斗业务 patch 层(工具边界+北斗插件;仅常规 profile,Safe Mode 不加载业务插件) */
  dshBeidouPatchPath?: string
...
export class HarnessRuntime {
  ...
  async start(launchDirectory: string, profile = 'web'): Promise<void> {
    ...
    const beidouPatchPath = profile !== SAFE_MODE_PROFILE ? this.options.dshBeidouPatchPath : undefined
    const patchPaths = [
      patchPath,
      ...(beidouPatchPath !== undefined && existsSync(beidouPatchPath) ? [beidouPatchPath] : []),
      ...(profile !== SAFE_MODE_PROFILE &&
    ...
    // Shell 环境会在 Electron 启动早期预热，而北斗工作区是在 bootstrap
    // 阶段才确定。必须把启动后注入的业务环境覆盖回 Harness 子进程，
    // 否则插件会回退到旧的 ~/Library/Application Support/北斗work/workspace。
    const runtimeEnvironment = {
      ...shellEnvironment,
      ...(process.env.BEIDOU_WORKSPACE !== undefined && { BEIDOU_WORKSPACE: process.env.BEIDOU_WORKSPACE }),
      ...(process.env.BEIDOU_ASSET_REPO !== undefined && { BEIDOU_ASSET_REPO: process.env.BEIDOU_ASSET_REPO }),
      ...(process.env.BEIDOU_ANYMETRICS_AUTH_VALUE !== undefined && { BEIDOU_ANYMETRICS_AUTH_VALUE: process.env.BEIDOU_ANYMETRICS_AUTH_VALUE }),
