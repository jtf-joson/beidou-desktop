...
import { appendFileSync, existsSync, readFileSync, writeFileSync, lstatSync, unlinkSync, symlinkSync, mkdirSync, realpathSync, cpSync } from 'node:fs'
...
/** 北斗插件装配(首启自动化):把包内 beidou-dsh-host 链接进 profile 共享树,
 * 使 build/beidou.patch.yml 的插件行可解析。真实文件系统目录(extraResources),
 * harness 子进程可直接加载;已链接或已安装(pnpm)时不动作。 */
/**
 * 北斗工作空间装配:确定 BEIDOU_WORKSPACE 并注入 harness 子进程环境。
 * 路径 = userData/beidou-workspace(与上游 userData/harness 布局对齐);
 * 首次启动从包内种子(build/beidou-workspace-seed)拷入默认资产;
 * 使用应用自己的 beidou-workspace，旧 Harness 工作区只保留在磁盘上，不自动导入。
 * model key 与 openId 由插件读 workspace config。
 */
function ensureBeidouWorkspace(): string {
  const preferred = join(app.getPath('userData'), 'beidou-workspace')
  let dir: string
  if (existsSync(preferred)) {
    ...
    try {
      ...
      const seed = app.isPackaged
        ? join(process.resourcesPath, 'beidou-workspace-seed')
        : join(app.getAppPath(), 'build/beidou-workspace-seed')
      if (existsSync(seed)) {
        for (const sub of ['semantics/import', 'knowledge', 'playbooks', 'asset-repo', 'asset-bundle']) {
          const src = join(seed, sub)
          const dst = join(dir, sub)
          if (!existsSync(dst) && existsSync(src)) {
            cpSync(src, dst, { recursive: true })
          }
        }
        const cfgSrc = join(seed, 'config.example.yaml')
        const cfgDst = join(dir, 'config.yaml')
        if (!existsSync(cfgDst) && existsSync(cfgSrc)) cpSync(cfgSrc, cfgDst)
      }
    } catch (error) {
      console.warn('[desktop] beidou workspace seed failed:', error)
    }
  }
  process.env.BEIDOU_WORKSPACE = dir
  // 开发运行时直接复用旁边的真实 Git 资产仓；打包运行时没有该目录，
  // 继续使用 userData 下的 Bundle/本地资产并按配置降级。
  const devRepoCandidates = [
    join(app.getAppPath(), '..', 'beidou-workspace'),
    join(process.cwd(), 'beidou-workspace'),
  ]
  const devRepo = devRepoCandidates.find((candidate) => existsSync(join(candidate, '.git')))
  if (devRepo) process.env.BEIDOU_ASSET_REPO = devRepo
  // DEEPSEEK_API_KEY:模型 key 注入(插件经 dsh-llm-deepseek 读取;无则模型不可用,工具仍可离线调)
  try {
    const configYaml = readFileSync(join(dir, 'config.yaml'), 'utf8')
    const modelMatch = /auth_token:\s*["']?([^"'\s#]+)/.exec(configYaml)
    if (modelMatch && !process.env.DEEPSEEK_API_KEY) {
      process.env.DEEPSEEK_API_KEY = modelMatch[1]!
    }
  } catch { /* 无 config.yaml:使用环境或已注入值 */ }
  console.log('[desktop] beidou workspace:', dir)
  return dir
}

function ensureBeidouPlugin(dshHome: string): void {
  for (const name of ['beidou-dsh-host', 'beidou-dsh-client']) {
    try {
      const source = app.isPackaged
        ? join(process.resourcesPath, name)
        : join(app.getAppPath(), 'packages', name)
      if (!existsSync(source)) continue
      if (name === 'beidou-dsh-host' && !existsSync(join(source, 'dist/index.js'))) continue
      const link = join(dshHome, 'profiles/node_modules', name)
      mkdirSync(dirname(link), { recursive: true })
      let stat: ReturnType<typeof lstatSync> | undefined
      try { stat = lstatSync(link) } catch { stat = undefined }
      if (stat?.isSymbolicLink()) {
        try { if (realpathSync.native(link) === realpathSync.native(source)) continue } catch { /* 重建 */ }
      } else if (stat !== undefined) {
        continue // pnpm 安装的真实目录(优先),不覆盖
      }
      try { unlinkSync(link) } catch { /* 不存在 */ }
      symlinkSync(source, link, 'junction')
      console.log(`[desktop] beidou plugin linked: ${name}`)
    } catch (error) {
      console.warn(`[desktop] beidou plugin link failed (${name}):`, error)
    }
  }
}
...
function launchHarness(): Promise<void> {
  ...
  harnessLaunchOperation = (async () => {
    ...
    // 北斗work 使用独立工作区和公司模型配置，不导入本机 DeepSeek Harness 的历史项目。
...
async function bootstrap(): Promise<void> {
  ...
  createWindow()
  ensureBeidouWorkspace()
  ensureBeidouPlugin(join(app.getPath('userData'), 'harness'))
  runtime = new HarnessRuntime({
    ...
    dshBeidouPatchPath: desktopResourcePath('beidou.patch.yml'),
